...
namespace KenticoInspector.Reports.ContentTreeConsistencyAnalysis
{
    public class Report : AbstractReport<Terms>
    {
        ...
        public override ReportResults GetResults() => CompileResults(
            GetNodeResult<CmsTreeNode>(
                Metadata.Terms.TableNames.TreeNodesWithABadParentSite,
                Scripts.GetTreeNodeIdsWithBadParentSiteId
                ),
            GetNodeResult<CmsTreeNode>(
                Metadata.Terms.TableNames.TreeNodesWithABadParentNode,
                Scripts.GetTreeNodeIdsWithBadParentNodeId
                ),
            GetNodeResult<CmsTreeNode>(
                Metadata.Terms.TableNames.TreeNodesWithLevelInconsistencyAliasPath,
                Scripts.GetTreeNodeIdsWithLevelMismatchByAliasPath
                ),
            GetNodeResult<CmsTreeNode>(
                Metadata.Terms.TableNames.TreeNodesWithLevelInconsistencyParent,
                Scripts.GetTreeNodeIdsWithLevelMismatchByNodeLevel
                ),
            GetNodeResult<CmsTreeNode>(
                Metadata.Terms.TableNames.TreeNodesWithNoDocumentNode,
                Scripts.GetTreeNodeIdsWithMissingDocument
                ),
            GetNodeResult<CmsTreeNode>(
                Metadata.Terms.TableNames.TreeNodesWithDuplicatedAliasPath,
                Scripts.GetTreeNodeIdsWithDuplicatedAliasPath
                ),
            GetNodeResult<CmsTreeNode>(
                Metadata.Terms.TableNames.TreeNodesWithPageTypeNotAssignedToSite,
                Scripts.GetTreeNodeIdsWithPageTypeNotAssignedToSite
                ),
            GetNodeResult<CmsDocument>(
                Metadata.Terms.TableNames.DocumentsWithNoTreeNode,
            ...
            where T : notnull
        {
            ...
            var details = databaseService.ExecuteSqlFromFile<T>(
                getDetailsSqlRelativeFilePath,
                new
                {
                    nodeIds
                }
        ...
        private ConsistencyResult? GetWorkflowResult()
        {
            ...
            foreach (var cmsClass in classItems)
            {
                ...
                if (!coupledDataIds.Any() || cmsClass.ClassTableName == null || cmsClass.ClassIDColumn == null) continue;
                ...
                var coupledData = databaseService.ExecuteSqlFromFile(
                ...
                comparisonResults.AddRange(
                    CompareVersionHistoryItemsWithPublishedItems(
                        classVersionHistoryItems,
                        coupledData,
                        cmsClass.ClassFields
                        )
                    );
            }
            ...
            if (comparisonResults.Any())
                return new ConsistencyResult
                {
                    Data = comparisonResults.AsResult()
                        .WithLabel(Metadata.Terms.TableNames.WorkflowInconsistencies),
        ...
        private IEnumerable<T> GetItemsWhereInManyIds<T>(
            ...
            )
        {
            ...
            foreach (var versionHistoryItem in versionHistoryItems)
            {
                var coupledDataItem = coupledData
                    .FirstOrDefault(
                        coupledDataRow =>
                            (int)coupledDataRow[idColumnName] == versionHistoryItem.CoupledDataID
                        );
                ...
                if (coupledDataItem != null && classFields != null)
                    foreach (var classField in classFields)
                    {
                        ...
                        var (versionHistoryValue, documentValue) = ProcessItemValues(
                            classField.ColumnType,
                            versionHistoryXmlValue,
                            coupledDataColumnValue
                            );
        ...
        private static (string? VersionHistoryValue, string? DocumentValue) ProcessItemValues(
            string columnType,
            string? versionHistoryXmlValue,
            object? coupledDataColumnValue
            )
        {
            if (versionHistoryXmlValue == null || coupledDataColumnValue == null) return (versionHistoryXmlValue, coupledDataColumnValue?.ToString());
            ...
            switch (columnType)
            {
                ...
                case FieldTypes.DateTime:
                    ...
                    var documentValueAdjusted = new DateTimeOffset(
                        (DateTime)coupledDataColumnValue,
                        versionHistoryValue.Offset
                        );

                    return (versionHistoryValue.ToString(), documentValueAdjusted.ToString());

                case FieldTypes.Boolean:
                    return (bool.Parse(versionHistoryXmlValue)
                            .ToString(),
                        Convert.ToBoolean(coupledDataColumnValue)
                            .ToString());
                ...
                case FieldTypes.Decimal:
                    ...
                    var formatting = "0.";
                    for (var i = 0; i < precision; i++) formatting += "0";

                    return (decimal.Parse(versionHistoryXmlValue)
                        .ToString(formatting), documentValueString);
